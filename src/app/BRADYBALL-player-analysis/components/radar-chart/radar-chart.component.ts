...
export class RadarChartComponent implements OnInit, OnChanges, AfterViewInit {
	...
	private createSvg(): void {
		...
		this.svg = d3
			.select(this.elementRef.nativeElement.querySelector('.radar-chart'))
			.append('svg')
	...
	saveSVG(): void {
		...
		clonedSvg.querySelectorAll('rect').forEach((rect) => {
			if (
				rect.getAttribute('width') === `${this.width}` &&
	...
	private createVintageFilter(): void {
		...
		const filter = defs.append('filter').attr('id', 'vintage-filter');
	...
	private createFeatureScales(dataPoints: RadarChartDataPoint[]): {
	...
	private drawSquiggle(): void {
		...
		const squiggleData = Array.from({ length: squigglePoints }, (_, i) => {
			...
			const noise =
				(Math.sin(angle * 20) + Math.sin(angle * 15) + Math.sin(angle * 10)) *
				(this.radius * 0.015);
			return [
	...
	private drawAxis(angle: number): void {
		...
		const squiggleData = Array.from({ length: squigglePoints }, (_, i) => {
			...
			const noise =
				(Math.sin(proportion * 20 * Math.PI) + Math.cos(proportion * 10 * Math.PI)) * 1.2;
			return [
		...
		this.chartGroup
			...
			.attr('d', lineFunction(squiggleData as [number, number][]))
			.attr('stroke', 'black')
			.attr('stroke-width', () => Math.random() * 0.8 + 0.5) // Varying thickness for hand-drawn effect
			.attr('fill', 'none');
	}

	private drawScaleLabels(
		point: RadarChartDataPoint,
		angle: number,
		featureScales: { [key: string]: d3.ScaleLinear<number, number> },
	): void {
		const circles = [0.2, 0.4, 0.6, 0.8, 1];
		circles.forEach((r, circleIndex) => {
			...
			text.each(function (this: SVGTextElement) {
				...
				const dy = (bbox.height / 2) * 0.2; // Adjust this multiplier as needed
	...
	private getRandomSize(circleIndex: number, numberOfCircles: number = 5): number {
		...
		if (circleIndex === outerCircleIndex) {
			...
			if (Math.random() < firstCircleLargerSizeProbability) {
				return firstCircleLargeSizes[
					Math.floor(Math.random() * firstCircleLargeSizes.length)
				];
			} else {
	...
	private drawLabel(point: RadarChartDataPoint, angle: number): void {
		...
		const textGroup = this.chartGroup
			.append('g')
			.attr(
				'transform',
				`translate(${labelX}, ${labelY}) rotate(${(angle * 180) / Math.PI})`,
			);
	...
	private createGradient(): void {
		...
		colors.forEach((stop) => {
			gradient
				...
				.attr('stop-opacity', stop.offset === '80%' || stop.offset === '90%' ? 0.9 : 0.85);
		});
	...
	private addNoiseFilter(): void {
		const filter = this.svg.append('defs').append('filter').attr('id', 'noise');
	...
	private drawDataArea(dataPoints: RadarChartDataPoint[]): void {
		const area = d3
			.areaRadial<RadarChartDataPoint>()
		...
		const dataAreaGroup = this.chartGroup.append('g').attr('class', 'data-area');

		// Create a clip path for the data area
		const clipPathId = 'dataAreaClip';
		const clipPath = this.svg.select('defs').append('clipPath').attr('id', clipPathId);

		clipPath.append('path').datum(dataPoints).attr('d', area);
	...
	private drawChalkBox(
		...
		const tempText = this.chartGroup
			.append('text')
			.attr('x', x)
			.attr('y', y)
			.text(text)
		...
		const strokeGroup = this.chartGroup
			.append('g')
