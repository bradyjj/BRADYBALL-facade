...
export class PercentileRankComponent implements OnInit, OnChanges, AfterViewInit {
	...
	private loadFonts(): void {
		const fontFiles = ['league-spartan-regular.woff2', 'league-spartan-bold.woff2'];
	...
	private createSvg(): void {
		...
		this.svg = d3
			.select(this.elementRef.nativeElement.querySelector('.gauge-container'))
			.append('svg')
	...
	private drawDescription(): void {
		...
		const descriptionGroup = this.svg
			.append('g')
			.attr('transform', `translate(${this.height + 50}, 60)`);

		const stickyNote = descriptionGroup.append('g').attr('transform', 'rotate(-1)');
		...
		const findOptimalFontSize = () => {
			...
			while (min <= max) {
				...
				words.forEach((word) => {
					...
					if (
						(tempText.node() as SVGTextContentElement).getComputedTextLength() >
						availableWidth
					) {
	...
	private createVintageEffects(): void {
		...
		ledGlow.append('feGaussianBlur').attr('stdDeviation', '2').attr('result', 'coloredBlur');
		...
		lcdScreen.append('rect').attr('width', '4').attr('height', '4').attr('fill', '#1a1a1a');
		...
		insetShadow.append('feOffset').attr('dx', '0').attr('dy', '4');

		insetShadow.append('feGaussianBlur').attr('stdDeviation', '4').attr('result', 'blur');
		...
		insetShadow.append('feFlood').attr('flood-color', 'black').attr('flood-opacity', '0.3');
		...
		digitalScreen.append('stop').attr('offset', '0%').attr('stop-color', '#000000');
		digitalScreen.append('stop').attr('offset', '50%').attr('stop-color', '#111111');
		digitalScreen.append('stop').attr('offset', '100%').attr('stop-color', '#000000');

		const screenPattern = defs
			.append('pattern')
		...
		needleGlow.append('feGaussianBlur').attr('stdDeviation', '3').attr('result', 'coloredBlur');
		...
		tickGlow.append('feGaussianBlur').attr('stdDeviation', '0.5').attr('result', 'blurOut');
		...
		const enhancedFaceGradient = defs
			.append('radialGradient')
		...
		const dotHighlight = defs.append('filter').attr('id', 'dotHighlight');
	...
	private drawGauge(): void {
		...
		const gaugeGroup = this.svg
			.append('g')
			.attr('class', 'gauge-group')
			.attr(
				'transform',
				`translate(${this.height / 2 + 28}, ${this.height / 2}) scale(${gaugeScale})`,
			);
	...
	private drawNeedle(group: any, value: number, startAngle: number, endAngle: number): void {
		...
		const needleGroup = group.append('g').attr('transform', `rotate(${angle})`);
	...
	private drawDigitalDisplay(group: any, value: number, yOffset: number = this.height / 3): void {
		const displayGroup = group.append('g').attr('transform', `translate(0, ${yOffset})`);
		...
		const segmentGroup = displayGroup.append('g').attr('transform', 'translate(0, 5)');
		...
		screwPositions.forEach(([x, y]) => {
			const screwGroup = displayGroup.append('g').attr('transform', `translate(${x},${y})`);
	...
	private addTubeEffects(defs: any): void {
		...
		tubeGlow.append('feGaussianBlur').attr('stdDeviation', '1').attr('result', 'coloredBlur');

		// Tube inner shadow for depth
		const tubeInnerShadow = defs
			.append('filter')
		...
		tubeInnerShadow.append('feGaussianBlur').attr('stdDeviation', '2').attr('result', 'blur');

		tubeInnerShadow.append('feOffset').attr('dx', '0').attr('dy', '2');

		// Liquid effect
		const liquidEffect = defs
			.append('filter')
		...
		liquidEffect.append('feGaussianBlur').attr('stdDeviation', '2').attr('result', 'blur');

		liquidEffect.append('feComposite').attr('operator', 'in').attr('in2', 'SourceGraphic');

		// Glass highlight gradient
		const glassHighlight = defs
			.append('linearGradient')
	...
	private createFiltersAndPatterns(defs: any): void {
		...
		innerShadow.append('feOffset').attr('dx', '0').attr('dy', '3');
	...
	private drawScreen(containerGroup: any): void {
		const screenGroup = containerGroup.append('g').attr('clip-path', 'url(#screenClip)');
	...
	private drawScrews(containerGroup: any): void {
		...
		screwPositions.forEach(([x, y]) => {
			const screwGroup = containerGroup.append('g').attr('transform', `translate(${x},${y})`);
	...
	private addGaugeEffects(defs: any): void {
		...
		gaugeGlow.append('feGaussianBlur').attr('stdDeviation', '3').attr('result', 'coloredBlur');
