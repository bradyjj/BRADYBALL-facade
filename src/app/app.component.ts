...
export class AppComponent implements AfterViewInit {
  ...
  private animate = () => {
    ...
    for (let i = this.comets.length - 1; i >= 0; i--) {
      ...
      const grad = this.ctx.createLinearGradient(c.x, c.y, c.x - c.vx * c.length, c.y - c.vy * c.length);
      ...
      if (c.alpha <= 0 || c.x < -100 || c.x > this.width + 100 || c.y < -100 || c.y > this.height + 100) {
