...
export class MenuComponent {
  ...
  getHistogramLines(): Array<{index: number, type: 'entry' | 'intermediate', position: number}> {
    const lines: Array<{index: number, type: 'entry' | 'intermediate', position: number}> = [];
  ...
  getDashedLineLength(lineInfo: {index: number, type: 'entry' | 'intermediate', position: number}): number {
    if (!this.menuOpen) {
