...
export class PitchComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    private scaleCoordinates(x: number, y: number): [number, number] {
        if (this.isVertical) {
            return [
                (y / 80) * this.PITCH_LENGTH,
                ((120 - x) / 120) * this.PITCH_WIDTH
            ];
        }
        return [
            (x / 120) * this.PITCH_LENGTH,
            (y / 80) * this.PITCH_WIDTH
        ];
    }
    ...
    private drawGoalSequence(goal: Goal) {
        ...
        const animateEvent = (index: number) => {
            ...
            const [startX, startY] = this.scaleCoordinates(event.start_location[0], event.start_location[1]);
            ...
            if (event.end_location) {
                if (event.event_type === 'pass') {
                    const path = eventGroup.append('path')
            ...
            // Draw start marker
            const startDot = eventGroup.append('circle')
                .attr('cx', startX)
                .attr('cy', startY)
                .attr('r', style.radius)
                .attr('fill', style.color)
            ...
            if (event.key_pass) {
                startDot.attr('fill', style.outlineColor)
                    .attr('stroke', style.color)
                    .attr('stroke-width', 0.5);
            }
            ...
            startDot.transition()
                ...
                .on('end', () => {
                    if (event.end_location && event.event_type !== 'shot') {
                        const endDot = eventGroup.append('circle')
                            .attr('cx', endX)
                            .attr('cy', endY)
                            .attr('r', style.radius)
                            .attr('fill', style.color)
                        ...
                        endDot.transition()
                            ...
                            .attr('opacity', 1)
                            .on('end', () => {
                                setTimeout(() => animateEvent(index + 1), ANIMATION_CONFIG.eventDelay);
                            });
                    } else {
     ...
     */
    private animateEventPath(event: BuildupEvent, startX: number, startY: number, endX: number, endY: number, onComplete: () => void) {
        const style = this.getEventStyle(event);
     ...
     */
    private animateCarryOrDribble(event: BuildupEvent, startX: number, startY: number, endX: number, endY: number, onComplete: () => void) {
        const style = this.getEventStyle(event);
        const path = this.pitch.append('path')
        ...
        const dot = this.pitch.append('circle')
            .attr('cx', startX)
            .attr('cy', startY)
            .attr('r', style.radius)
            .attr('fill', style.color);
     ...
     * Animate pass events
     */
    private animatePass(event: BuildupEvent, startX: number, startY: number, endX: number, endY: number, onComplete: () => void) {
        const style = this.getEventStyle(event);
     ...
     * Animate shot events
     */
    private animateShot(event: BuildupEvent, startX: number, startY: number, endX: number, endY: number, onComplete: () => void) {
        const style = this.getEventStyle(event);
        const path = this.pitch.append('path')
            .attr('d', this.createCurvedPath(startX, startY, endX, endY))
            .attr('fill', 'none')
    ...
    private drawPitchBackground() {
        ...
        const wornGradient = defs.append('radialGradient')
            .attr('id', 'wornSpot')
    ...
    private drawFieldLines() {
        ...
        [0, this.PITCH_LENGTH].forEach(x => {
            ...
            this.pitch.append('path')
                .attr('d', arcGenerator({} as any))
                .attr('transform', `translate(${isLeft ? 11 : this.PITCH_LENGTH - 11},${this.PITCH_WIDTH / 2})`)
                .attr('stroke', this.LINE_COLOR)
                .attr('fill', 'none')
                .attr('stroke-width', 0.5);
